...
func MergeSortAsync() {
	...
	slog.Info("")
	slog.Info("======> MergeSortAsync")

	data := []int{9, 4, 3, 6, 1, 2, 10, 5, 7, 8}
	fmt.Printf("%v\n%v\n", data, mergeSortAsync(data))
}

func mergeAsync(left, right []int) []int {
...
func mergeSortAsync(data []int) []int {
	if len(data) <= 1 {
		return data
	}
	done := make(chan bool)
	...
	var left []int
	go func() {
		left = mergeSortAsync(data[:mid])
		done <- true
	}()
	right := mergeSortAsync(data[mid:])
	<-done
	return mergeAsync(left, right)
}
